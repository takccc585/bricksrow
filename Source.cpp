...
int main()
{
	...
	//блоки
	BrickRow brickrow;
	brickRowInint(brickrow, 20 , sf::Vector2f(0,0), StepX);
	//счет
	int score = 0;
	Font font;
	...
	while (window.isOpen())
	{
		...
		if ((midLowY >= brick.getPosition().y+ BRICK_HEIGHT) and (midLeftX + ballRadius >= r.getPosition().x) and (midRightX <= r.getPosition().x + sizew)) {
			dy = -dy;
			score++;
			right_score_text.setString(to_string(score));
		}
